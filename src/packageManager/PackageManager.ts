...
export async function DownloadAndInstallPackages(packages: Package[], provider: NetworkSettingsProvider, platformInfo: PlatformInformation) {
    let filteredPackages = await filterPackages(packages, platformInfo);
    if (filteredPackages) {
        for (let pkg of filteredPackages) {
            try {
                let buffer = await DownloadFile(pkg.description, provider, pkg.url, pkg.fallbackUrl);
                await InstallZip(buffer, pkg.description, pkg.installPath, pkg.binaries, pkg.links);
            } catch (error) {
